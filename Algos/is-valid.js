...
function isValid(stale, latest, otjson) {
    if (stale === latest && otjson === '[]') return true;
    ...
    for (let op of operations) {
        if (op.op === 'skip') {
            currPosition += op.count;
        } else if (op.op === 'delete') {
            if (currPosition + op.count > result.length) return false; 
            result.splice(currPosition, op.count);
        } else if (op.op === 'insert') {
            result.splice(currPosition, 0, ...op.chars.split(''));
            currPosition += op.chars.length;
        }

        if (currPosition > result.length) return false;
    }
